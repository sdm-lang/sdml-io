...
of the sequence has a leading ~#~ character to differentiate from other sequence values. Note that whitespace is entirely
optional, but recommended to layout the hex values.
...
#+CAPTION: One-Of Feature Set
#+BEGIN_SRC sdml :exports code :noeval
module example is

  features AccountKind {1} of
...
#+CAPTION: Some-Of Feature Set
#+BEGIN_SRC sdml :exports code :noeval
module example is

  features AccountUpgrades {1..} is
    SkipCheckin
    AutoUpgrade
  end
