...
[[#unicode-usage]] for details.

For the complete grammar, and notes on BNF notation, see section [[#surface-syntax-grammar]].

** Whitespace and Comments
:PROPERTIES:
:CUSTOM_ID: syn:whitespace-and-comments
:END:
...
:PROPERTIES:
:CUSTOM_ID: syn:qualified-identifiers
:END:
...
:PROPERTIES:
:CUSTOM_ID: syn:modules-and-imports
:END:
...
2. If the reader has access to an SDML catalog file (For details of the catalog file format see appendix
   [[#impl:resolver-catalog]]), a matching catalog entry MUST be used as the module URI.
...
Data types are further described in section [[#syn:datatypes-and-values]]; entities, enumerations, events, structures, and disjoint
unions are described in section [[#syn:structured-types]]; and annotation properties are described in section [[#syn:annotation-properties]].
...
:PROPERTIES:
:CUSTOM_ID: syn:datatypes-and-values
:END:
...
*** Booleans
:PROPERTIES:
:CUSTOM_ID: syn:datatype-boolean
:END:
...
*** Unsigned Integers
:PROPERTIES:
:CUSTOM_ID: syn:datatype-unsigned
:END:
...
*** Signed Integers
:PROPERTIES:
:CUSTOM_ID: syn:datatype-integer
:END:
...
*** Decimal Numbers
:PROPERTIES:
:CUSTOM_ID: syn:datatype-decimal
:END:
...
*** Value Constructors
:PROPERTIES:
:CUSTOM_ID: syn:value-constructor
:END:
...
You can ignore the syntax of ~@ex:thing~ which is an annotation, see section [[#syn:annotations]], used to ensure the syntax is
complete for all examples.

*** Mapping Values
:PROPERTIES:
:CUSTOM_ID: syn:mapping-values
:END:
...
*** Sequence Values
:PROPERTIES:
:CUSTOM_ID: syn:sequence-values
:END:

As stated in section [[#syn:modules-and-imports]], sequences in SDML start with the Left Square Bracket ("[", =U+005B=)
...
*** Defining Data Types
:PROPERTIES:
:CUSTOM_ID: syn:defining-datatypes
:END:
...
A /dictionary/ or /map/ structure can be described in SDML as a sequence of mapping values (see section [[#syn:mapping-values]]).
...
:PROPERTIES:
:CUSTOM_ID: syn:annotation-properties
:END:
...
has an identifier that resolves to an OWL annotation property (see section [[#syn:defining-rdf-classes-and-properties]] for the detailed rules), and a
...
*** Constraints
:PROPERTIES:
:CUSTOM_ID: syn:annotation-constraints
:END:

This section will only briefly introduce constraints as section [[#model-constraints]] provides a detailed description.
...
*** Defining RDF Classes and Properties
:PROPERTIES:
:CUSTOM_ID: syn:defining-rdf-classes-and-properties
:END:
...
** Structured Types
:PROPERTIES:
:CUSTOM_ID: syn:structured-types
:END:
...
identifier for this entity but is also used in event sources (see section [[#syn:entity-events]]).
...
The syntax for mapping values was previously described in section [[#syn:mapping-values]].

*** Member Cardinality
:PROPERTIES:
:CUSTOM_ID: syn:member-cardinality
:END:
...
in a sequence type (see section [[#syn:sequence-types]]).
...
*** Property Reference
:PROPERTIES:
:CUSTOM_ID: syn:property-reference
:END:
...
:PROPERTIES:
:CUSTOM_ID: syn:property-definitions
:END:
...
#+CAPTION: Example: Property Definition
#+BEGIN_SRC sdml :exports code :noeval
module account is

  datatype Accountid <- unsigned

  property accountId -> {1} AccountId is
    @skos:prefLabel = "account identifier"@en
    @skos:definition =
