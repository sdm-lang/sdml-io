...
3. Add a new section to the =index.org= file with the [[index-entry][block of code]] below.
4. Create a new pull request with these changes and label as "RFC".
...
strict digraph {
    ...
    node [shape="ellipse"; fontname="Sans-Serif"; fontsize=12; width=1.5];

    // All place nodes.
    start [style="filled"; color="black"; fillcolor="black"; label=""; height=0.2; width=0.2];
    draft [label="Draft"];
...
specified if it makes sense. The current tags are =grammar=, =binding=, =test=, =query=, =example=, and =doc=.
