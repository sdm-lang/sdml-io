...
the use of the arrow operator ~->~ or ~→~ that denotes the type (right-hand side) of a member (left-hand side).
...
It is also time to deal with the ~import~ statement that we have already seen in a number of examples.
...
#+NAME: tbl:datatype-comparisons
#+CAPTION: Datatype Comparisons
| Datatype     | Builtin Constraints                     | SQL Equivalent                      | Java Equivalent         | Rust Equivalent        |
|--------------+-----------------------------------------+-------------------------------------+-------------------------+------------------------|
| ~binary~       | ~minLength~, ~maxLength~, and ~length~        | ~BINARY(length)~ or ~VARBINARY(length)~ | ~byte[length]~            | ~Vec<u8>~ or ~[u8;length]~ |
| ~boolean~      |                                         | ~BIT~                                 | ~boolean~                 | ~bool~                   |
| ~decimal~      | ~totalDigits~, ~fractionDigits~, and /scalar/ | ~DECIMAL(total, fraction)~            | ~BigDecimal~ in ~java.math~ |                        |
| ~double~       | /scalar/                                  | ~DOUBLE PRECISION~                    | ~double~                  | ~f64~                    |
| ~integer~      | /scalar/                                  | ~INTEGER~                             | ~long~                    | ~i64~                    |
| ~string~       | ~minLength~, ~maxLength~, and ~length~        | ~CHAR(length)~ or ~VARCHAR(length)~     | ~String~                  | ~String~ or ~str~          |
| ~unsigned~     | /scalar/                                  |                                     |                         | ~u64~                    |
| ~uri~          | ~minLength~, ~maxLength~, and ~length~        |                                     |                         |                        |
| ~xsd:date~     | ~explicitTimezone~ and /scalar/             | DATE                                |                         |                        |
| ~xsd:time~     | ~explicitTimezone~ and /scalar/             | TIME                                |                         |                        |
| ~xsd:dateTime~ | ~explicitTimezone~ and /scalar/             | DATE                                |                         |                        |
| ~xsd:duration~ | ~explicitTimezone~ and /scalar/             | INTERVAL                            |                         |                        |
