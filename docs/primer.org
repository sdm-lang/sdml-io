...
An enumeration is a list of unique variants that represent unique options.

*** Retail Example

In listing [[src:initial-retail-entities]] we have added a common ~Address~ structure that is used by both the customer and
store entities. This example does not introduce an enumeration directly; however, the imported type
...
module hospitality is
  ...
  union PaymentInstrument of
    assert only_one = "one, and only one of upc, ean, or isbn may be present"
    Currency as Cash
