...
/restricts/ the basic string to between 8 and 13 characters only using a regex pattern.
...
Table [[tbl:datatype-comparisons]] lists the constraints that can you can use to restrict existing types. All /scalar/ types
have the following additional constraints; ~maxInclusive~, ~maxExclusive~, ~minInclusive~, and ~minExclusive~.
...
module logistics is
  ...
  event CarrierLoadTender source Consignor ;; X.12 204

  entity Carrier

  event CarrierLoadTenderResponse source Carrier ;; X.12 990
  ;; PickUpManifest X.12 5
  ...
  event BillOfLading source Consignment X.12 211
  event FreightDetailsAndInvoice source Consignment ;; X.12 210
  ...
  event AdvanceShippingNotice source Shipment ;; X.12 856
;; ShippingSchedule X.12 862
  event TransportStatusNotification source Shipment ;; X.12 214
